...
image::https://upload.wikimedia.org/wikipedia/commons/7/79/Hans_Burgkmair_d._%C3%84._%28zugeschr.%29_-_Bildnis_Kaiser_Friedrich_III.jpg[]
...
[source,cypher]
----
LOAD CSV WITH HEADERS FROM "file:///home/stefan/neo/customers/kuczera/tagung/akdwmainz_fiii_guide/csv/regesten.csv" AS line
UNWIND apoc.text.regexGroups(line.archival_history, "<link (\\S+)>(\\S+)</link>") as link
MATCH (r:Regest {regid:line.persistent_identifier})
